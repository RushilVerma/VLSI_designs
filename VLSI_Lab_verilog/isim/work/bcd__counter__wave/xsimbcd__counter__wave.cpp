...
va_list vap)
{
    extern HSim__s6 * createworkMbcd__counter__wave(const char*);
    HSim__s6 *blk = createworkMbcd__counter__wave(label); 
    return blk;
}
...
va_list vap)
{
    extern HSim__s6 * createworkMbcd__counter(const char*);
    HSim__s6 *blk = createworkMbcd__counter(label); 
    return blk;
}
...
public:
    ...
    HSimConfigDecl * topModuleInstantiate() {
        ...
        (*cfgvh).addVlogModule("work","bcd_counter_wave", (HSimInstFactoryPtr)IF0);
        (*cfgvh).addVlogModule("work","bcd_counter", (HSimInstFactoryPtr)IF1);
        (*cfgvh).addVlogModule("work","glbl", (HSimInstFactoryPtr)IF2);
        HSim__s5 * topvl = 0;
        extern HSim__s6 * createworkMbcd__counter__wave(const char*);
        topvl = (HSim__s5*)createworkMbcd__counter__wave("bcd_counter_wave");
